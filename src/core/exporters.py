...
def extract_specific_file_types(
    hex_data: HexData,
    analysis_result: AnalysisResult,
    file_types: List[str],
    output_dir: str = "result"
    ...
    Args:
        ...
        file_types (List[str]): File types to extract (e.g., ['JPEG', 'PNG'])
        output_dir (str): Output directory
    
    Returns:
        ExtractionResult: Extraction results
    """
    return extract_detected_files(
        hex_data=hex_data,
...
def get_extraction_summary(extraction_result: ExtractionResult) -> Dict[str, any]:
    """
    Get summary statistics of extraction operation.
    
    Args:
        extraction_result (ExtractionResult): Extraction results
    ...
    for extracted in extraction_result.extracted_files:
        ...
        total_size += extracted['size']
    
    return {
        'total_files': extraction_result.total_extracted,
        'total_size_bytes': total_size,
        'total_size_mb': total_size / (1024 * 1024),
        'success_rate': extraction_result.success_rate,
        'types_extracted': type_counts,
        'output_directory': extraction_result.output_directory,
        'failed_count': len(extraction_result.failed_extractions)
    }
